<?php
  ...
  try {
      if (isset($_SESSION['user']['id'])) {
        ...
        $arr['club_classid'] = !empty($_GET['club_class_id']) ? $_GET['club_class_id'] : '';
        ...
        $arr['Class_Start_Time'] = date('g:i A', strtotime( $start_time)) ;
        $end_time = !empty($_POST['InputEndTime']) ? $_POST['InputEndTime'] : '';
        //$arr['Class_End_Time'] = date('H:i:s', $end_time);
        $arr['Class_End_Time'] = date('g:i A', strtotime( $end_time)) ;
        $arr['Updated_Date'] = date('Y-m-d H:i:s');
