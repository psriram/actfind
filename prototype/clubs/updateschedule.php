<?php
  ...
  try {
      if (isset($_SESSION['user']['id'])) {
        ...
        $where = sprintf('class_id=%d', $_GET['class_id']);

        $result = $modelGeneral->updateDetails('Class_Setup', $arr, $where);
        ...
        $where = sprintf('class_schedule_id=%d', $_GET['class_schedule_id']);
        if (!empty($class_day)) {

           foreach ($class_day as $v) {
              $arr['Class_Day']  = $v;

              $Models_General->updateDetails('Class_Schedule', $arr);
            }
        }
